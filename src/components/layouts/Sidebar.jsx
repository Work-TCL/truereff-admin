  ...
  return (
    <aside className="w-60 min-h-screen bg-gray-900 text-white p-4">
      ...
      <ul className="space-y-1">
        {navItems.map((item) => (
          <li key={item.name} className={`flex hover:bg-white ${active === item?.name ? "bg-white text-black":""} items-center gap-3 cursor-pointer hover:text-black p-2 rounded-lg`} onClick={() => handleActive(item)}>
            {item.icon}
            <span>{item.name}</span>
          </li>
        ))}
